...
export default function HomeDashboard() {
  ...
  const dataSources = [
    ...
    { name: "Lifestyle & Biomarkers", count: "12 Parameters", lastSync: "Live Twin Sync", status: "Verified", icon: TbActivityHeartbeat },
  ...
  const clinicalSummaryItems = masterInsights
    ? Object.entries(masterInsights).map(([organ, data]) => {
        const isOrganReady = hasUploadedRecords && (data.is_active || (data.numerical_score !== null && data.numerical_score !== undefined)) && data.top_factor !== "system_busy";
        return {
    ...
    : [
        {
          ...
          reason: "Clinical guidance is generated exclusively from verified diagnostic laboratory biomarkers.",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {masterInsights ? (
              Object.entries(masterInsights).map(([organ, organData]) => {
                ...
                console.log(`[PIPELINE 15] HomeDashboard organ render: organ=${organ}`, {
                  hasUploadedRecords,
                  is_active: data.is_active,
                  numerical_score: data.numerical_score,
                  top_factor: data.top_factor,
                  isReadyComputed: isReady
                });
                          ...
                          <div>
                            ...
                            <div className="flex items-baseline gap-1.5 mt-1">
                              ...
                              <span className="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 ml-auto bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 px-2 py-0.5 rounded-md">
                                {data.confidence_level || "Biomarker Verified"}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="bg-white dark:bg-slate-800/80 p-4 rounded-xl border border-amber-200/80 dark:border-amber-900/40 space-y-1.5 shadow-2xs">
                            ...
                            <p className="text-xs text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                              {data.explanation && !data.explanation.includes("System busy") 
                                ? data.explanation 
                                : `Laboratory biomarkers (${organ === "heart" ? "Lipid Profile, BP" : organ === "kidneys" ? "Serum Creatinine, eGFR" : organ === "liver" ? "Bilirubin, INR, ALT/AST" : organ === "lungs" ? "Spirometry FEV1/FVC" : "BP, Heart Rhythm"}) are required to calculate an evidence-based clinical assessment.`}
                            </p>
                          </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 md:p-8 rounded-3xl text-white shadow-md flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="space-y-2 max-w-xl">
                ...
                <p className="text-xs text-blue-100 font-medium leading-relaxed">
                  Interactive 3D rendering mapping real-time biomarker stress levels across cardiovascular, pulmonary, renal, hepatic, and neurological organ systems.
                </p>
              </div>
              <Link
                to="/3d-twin"
